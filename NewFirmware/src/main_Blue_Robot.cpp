...
void setup()
{
  ...
  vac_Off;   // Vacuum OFF
  blast_Off; // Blast OFF
  cyl_Down;    // Cylinder UP

  for (U8 k = 0; k < stepperAmount; k++) Axis[k].Homed = false;
  ...
  SerialUSB.setTimeout(10); // set shorter timeout for serial read, default is 1000ms but we don't/want need that long for this application
...
void loop()
{
  if (SerialUSB.available())
  {
    ...
    String rx = SerialUSB.readStringUntil('\n');
    if (rx.indexOf("STOP") == -1) CommandBuffer[CB_WriteIndex++] = rx;
    else
    {
      for (int i = 1; i < 4; i++) steppers[i]->stop();
      SerialUSB.println("STOPPED");
      ClearBuffer();
    }

    if (echo_commands) SerialUSB.println("RECEIVED: " + rx);
  }
  ...
  // Check if the arm is moving by ORing the distances to go
  inMove =  steppers[1]->distanceToGo() || 
            steppers[2]->distanceToGo() || 
            steppers[3]->distanceToGo() ;
}
...
{
  for (U8 k = 1; k < stepperAmount; k++) steppers[k]->run();
}
...
void process_orders(String content)
{
  ...
  else if (CurrentCommand[0] == "MOVE") // Moves the robot arm from its current position to that indicated
  {
    ...
    {
      strcpy(char_array[k], CurrentCommand[k].c_str());
    }
    ...
    SerialUSB.println("MOVE: " + String(CurrentCommand[1]) + ' ' + String(CurrentCommand[2]) + ' ' + String(CurrentCommand[3]));
  }
  else if (CurrentCommand[0] == "ID") // Asks if the robot arm is right- or left-handed
  {
    if (Robot == 1) SerialUSB.println("ID, ROBOTL");
    else SerialUSB.println("ID, ROBOTR");
  }
  ...
  else if (CurrentCommand[0] == "OFFSET") // Sets the offsets for the three stepper axes
  {
    ...
    for (U8 k = 0; k < 15; k++)
    {
      if (CurrentCommand[1] == Driver[k].ON_OFF)
      {
        Curent_Offset = k;
      }
    }
  ...
  else if (CurrentCommand[0] == "SPEEDSET") // Sets the maximum speed and acceleration of the robot arm to a percentage of the default values
  {
    ...
    for (U8 k = 1; k < 3; k++)
    {
      strcpy(char_array_s[k], CurrentCommand[k].c_str());
    }
    ...
    SerialUSB.println("SPEEDSET: " + CurrentCommand[1] + " " + CurrentCommand[2]);
  }
  else if (CurrentCommand[0] == "WAIT")
  {
    uint32_t del = CurrentCommand[1].toInt();
    LockoutTime = millis() + del;
    SerialUSB.println("WAIT: " + String(del));
  }
  else if (CurrentCommand[0] == "DING")
  {
    SerialUSB.println("DONG: " + String(CurrentCommand[1]));
  }
  else if (CurrentCommand[0] == "ECHO")
  {
    echo_commands = CurrentCommand[1].toInt();
    SerialUSB.println("ECHO: " + String(echo_commands));
  }
  ...
  {
    SerialUSB.println("NACK: " + String(CurrentCommand[0]));
  }
}
...
void air_process(String air_command):
  1. The string air_command is the string that has been received from the serial port
  ...
  9. If the string is not equal to any of the above, then the function will set the string Msg_Air to "NO" + air_command.
